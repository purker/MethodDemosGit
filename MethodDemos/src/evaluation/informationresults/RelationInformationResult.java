 ...
 */
public class RelationInformationResult
		extends AbstractSingleInformationResult<Set<RelationInformationResult.StringRelation>> {
	private Comparator<String> comp2;

	public RelationInformationResult(EvalInformationType type, Set<StringRelation> expected,
			Set<StringRelation> extracted) {
	...
	public void evaluate() {
		...
		if (hasExpected())
			recall = new BigDecimal(correctCount).divide(new BigDecimal(expectedValue.size()), Config.bigDecimalScale,
					Config.bigDecimalRoundingMode);
		if (hasExtracted())
			precision = new BigDecimal(correctCount).divide(new BigDecimal(extractedValue.size()),
					Config.bigDecimalScale, Config.bigDecimalRoundingMode);
		...
		if (hasExpected() && hasExtracted()) {
			...
			external: for (StringRelation partExt : extractedValue) {
				for (StringRelation partExp : tmp) {
					if (comp.compare(partExt.element1, partExp.element1) == 0
